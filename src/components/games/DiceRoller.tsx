  ...
  const handlePlay = () => {
    ...
    setTimeout(() => {
      if (roll > 3) {
        ...
        toast.success(`You rolled a ${roll}! You won $${winAmount - bet}!`);
      } else {
        playLoseSound();
        setStats(prev => ({ ...prev, losses: prev.losses + 1 }));
        toast.error(`You rolled a ${roll}. Better luck next time!`);
      }
      setIsPlaying(false);
    }, 1000);
  };
  ...
  return (
    <Card className="p-6 space-y-6 bg-casino-black/90 border-casino-gold/20">
      <div className="text-center">
        <h2 className="text-2xl font-bold text-casino-gold mb-2">Dice Roller</h2>
        <p className="text-sm text-gray-400">Roll 4 or higher to win!</p>
